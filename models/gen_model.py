...
class Generator(nn.Module):
    ...
    def __init__(self, noise_dim):
        ...
        self.tanh = nn.Tanh()
        self.inp = GenBlock(in_channels=self.noise_dim, out_channels=256, upsample=False, inp=True) # 512x4x4
        self.conv1 = GenBlock(in_channels=256, out_channels=128) # 512x8x8
        self.conv2 = GenBlock(in_channels=128, out_channels=64) #5
        self.conv3 = GenBlock(in_channels=64, out_channels=32)
        self.conv4 = GenBlock(in_channels=32, out_channels=16)
    ...
    def forward(self, x, step, alpha):
        ...
        for i in range(step+1):
            if 0 <= alpha < 1 and i == step and step > 1:
                prev_x = x
